    ...
    <div class="row">
        ...
        <div class="col-md-12">
            <div class="">
                ...
                <h3 class="title-5 m-b-20"> Flight</h3>
                                    ...
                                    <td class="uploader">
                                        ...
                                        @else
                                            @foreach($files as $file)
                                                <i onclick="removeFile('{{ $file }}');" class="fa fa-times text-danger pull-right p-1"></i>
                                                <i onclick="window.open('/key-travel/{{ $file }}/file/')" class="fa fa-file-pdf attached" />
                                            @endforeach
                                        @endif
                                    </td>
                                </tr>
                ...
                <div class="col-md-8">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-8">
                            <input name="from" value="{{ $item->from }}" class="form-control required" />
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-8">
                            <input name="to" value="{{ $item->to }}" class="form-control required" />
                        </div>
                    </div>
                    ...
                    <div class="col-md-12 text-right">
                        <hr>
                            <button type="button" onclick="complete();" class="btn btn-primary complete-btn">Close Order</button>
                    </div>
...
@section('js')
    <script>
        ...
        function upload(input) {
            ...
            $.ajax({
                url: '/key-travel/{{ $order->order_id }}' + '/' + id + "/upload",
                type:"POST",
                processData:false,
                contentType: false,
                data: formData,
        ...
        function complete(){
            ...
            $.get('/key-travel/{{ strtolower($order->type) }}/{{ $order->order_id }}/complete',function(response){
                Swal.close();
                if(response.success){
                    Swal.fire(
                        'Success',
        ...
        function notes(item){
            $.get('/key-travel/flight/{{ $order->order_id }}/'+item+'/notes',function(response){
        ...
        function saveNote(item){
            ...
            $.post('/key-travel/flight/{{ $order->order_id }}/'+item+'/notes','_token={{ csrf_token() }}&note='+encodeURIComponent(note),function(response){
