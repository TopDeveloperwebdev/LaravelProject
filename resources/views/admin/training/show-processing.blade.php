                        ...
                        <small>
                            <table class="table table-bordered table-small">
                                ...
                                <tr>
                                    <td>Req Attached</td>
                                    <td class="uploader">
                                        @if(!$order->file && !$order->completed_at)
                ...
                <form method="post" action="{{ route('training.store') }}" class="form" enctype="multipart/form-data">
                    ...
                    <div class="col-md-12 text-right">
                        ...
                        @if(!$order->completed_at)
                            <button type="button" onclick="submitOrder();" class="btn btn-primary">Close Requisition</button>
                        @endif
                    </div>
                </form>
            </div>
...
@section('js')
    <script>
        ...
        function submitOrder(){
            ...
            $.get("/training/{{ $order->order_id }}/complete",function(response){
                Swal.close();
                if(response.success){
                    Swal.fire(
                        'Success',
