                ...
                <div class="table-responsive table-responsive-data2 m-b-20">
                    ...
                    <thead>
                                <tr>
                                    ...
                                    <th>Files</th>
                                    <th>Notes</th>
                                    <th>@if(!$order[0]->completed_at)<i class="fa fa-times text-danger"></i>@endif</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($order as $item)
                                    <tr id="{{ $item->id }}">
                                        ...
                                        <td>
                                            <select disabled name="type_id" onchange="selChange(this)">
                                        ...
                                        <td>@if(!$item->completed_at)<i onclick="removeItem({{ $item->id }})" class="fa fa-times text-danger"></i>@endif</td>
                                    </tr>
                                    
                                    <script>
                                        files[{{ $item->id }}] = {!! $item->files !!};
                                    </script>
                                @endforeach
    ...
    <script>
        ...
        function onEditFinish(input){
            ...
            $("#saveBtn").click(function(){
                $.post('/none-catalogue/{{ $order[0]->order_id }}/' + id ,data,function(response){
        ...
        function complete(){
            ...
            $.get('/none-catalogue/{{ $order[0]->order_id }}/processing',function(response){
                Swal.close();
                if(response.success){
                    Swal.fire(
                        'Success',
