                        ...
                        <small>
                            <table class="table table-bordered table-small">
                                ...
                                <tr>
                                    ...
                                    <tr><td>Order Submit Date</td><td>{{ $order->first()->first()->created_at->format('d/m/y') }}</td></tr>
                        ...
                        @foreach($order as $items)
                            @foreach($items as $item)
                                <tr id="{{ $item->id }}" data-price="{{ $item->price }}">
                                    ...
                                    <td width="20%"><input onchange="total(this)" value="{{ $item->qty }}" class="form-control" /></td>
                                    @if($order->first()->first()->placed_at)
                                    <td><input value="{{ $item->qty_received }}" class="form-control" /></td>
                                    @endif
...
@section('js')
    <script>
        ...
        $('input').change(function(){
            ...
            $(this).closest('tr').each(function(){
                 data = {
                    ...
                    'qty_received': $(this).find('td').eq(3).find('input').val(),
                    'total': $(this).find('td').eq(3).find('input').val() * parseFloat($(this).attr('data-price'))
                    @endif
                };
                id = $(this).attr('id')
        ...
        function submitOrder(){
            ...
            $.get("/stores/{{ $order->first()->first()->order_id }}/processing",function(response){
                Swal.close();
                if(response.success){
                    Swal.fire(
                        'Success',
        ...
        function total(input){
            var qty = parseInt($(input).closest('tr').find('td').eq(2).find('input').val());
