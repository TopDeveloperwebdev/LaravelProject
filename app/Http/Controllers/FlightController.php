...
class FlightController extends Controller
{
    ...
    public function show($order)
    {
        ...
        if (auth()->user()->is_admin && auth()->user()->admin_view) {
            if (!$order->placed_at) {
                return view('admin.travel.flight.show', ['order' => $order]);
            }
            return view('admin.travel.flight.show-processing', ['order' => $order, 'files' => $files]);
        }

        if (!$order->placed_at) {
            return view('travel.flight.show', ['order' => $order]);
        }

        return view('travel.flight.show', ['order' => $order, 'history' => true, 'files' => $files]);
    }
