...
class TrainController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $item['ltc_14'] = isset($item['ltc_14']) ? 1 : 0;
        $item['ltc_16'] = isset($item['ltc_16']) ? 1 : 0;
        ...
        if (!$new) {
            ...
            // auth()->user()->keyTravel()->where('order_id', $order_id)->delete();
            // auth()->user()->trainOrders()->where('order_id', $order_id)->delete();
            throw new Exception('An Unknown error has occured', 1);
        }
    ...
    public function show($order)
    {
        ...
        if (auth()->user()->is_admin && auth()->user()->admin_view) {
            if (!$order->placed_at) {
                return view('admin.travel.train.show', ['order' => $order]);
            }

            return view('admin.travel.train.show-processing', ['order' => $order, 'files' => $files]);
        }

        if (!$order->placed_at) {
            return view('travel.train.show', ['order' => $order]);
        }

        return view('travel.train.show', ['order' => $order, 'history' => true, 'files' => $files]);
    ...
    public function update(Request $request, $order, $item)
    {
        ...
        return ['success' => $item->update($data) ? true : false];
    }
